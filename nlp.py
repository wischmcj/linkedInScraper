...
if __name__ == "__main__":
    ...
    def extract_common_ngrams(df, field='description', n=2, top_k=20):
        ngram_counter = Counter()
        for desc in df[field].dropna():
            clean_text = preprocess_text(str(desc).lower())
            ngrams = get_ngrams(clean_text, n)
            ngram_counter.update(ngrams)
        return ngram_counter.most_common(top_k)

    print("Top bigrams in data_jobs:")
    print(extract_common_ngrams(data_jobs, field='description', n=2, top_k=200))
